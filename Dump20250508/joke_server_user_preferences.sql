...
CREATE TABLE `user_preferences` (
  ...
  `preference_type` enum('like','dislike','favorite','block') NOT NULL,
  `preference_strength` int NOT NULL DEFAULT '1',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`preference_id`),
  UNIQUE KEY `uq_user_category_pref` (`user_id`,`category_id`,`preference_type`),
  KEY `idx_user_preferences_user` (`user_id`),
  KEY `idx_user_preferences_category` (`category_id`),
  CONSTRAINT `fk_user_preferences_category` FOREIGN KEY (`category_id`) REFERENCES `categories` (`category_id`) ON DELETE CASCADE,
